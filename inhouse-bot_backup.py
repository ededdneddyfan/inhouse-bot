...
async def lockmap(ctx):
    ...
    if(mapVote == True):
        ...
        if(winningMap == "New Maps"):
            ...
            await ctx.send("185.107.96.189:27015 and the password is: " + SERVER_PASSWORD)
            await DePopulatePickup(ctx)
...
async def forcestats(ctx):
    ...
    if ctx.channel.name == 'pickup':
        ...
        sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        sock.sendto("BOT_MSG@END".encode(), ('185.107.96.189', int(CLIENT_PORT)))
...
@client.command(pass_context=True)
async def hltv(ctx):
    await ctx.send("HLTV: currently not operational, check back later!")
...
async def server(ctx):
    await ctx.send("steam://connect/185.107.96.189:27015/%s" % SERVER_PASSWORD)
